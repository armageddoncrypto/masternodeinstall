# Armageddon Masternode Install Script
...
4. Send 1000 AGDN to this address
...
```
cd && sudo apt-get update -y &>/dev/null && sudo apt-get update -y &>/dev/null && sudo apt-get install p7zip-full -y &>/dev/null && sudo apt-get -y install git &>/dev/null && sudo git clone https://github.com/Armageddoncrypto/masternodeinstall &>/dev/null && cd masternodeinstall/ &>/dev/null && sudo bash install.sh && cd && rm -r masternodeinstall/ 
```
...
```<alias> <ip>:11800 <private_key> <tx_id> <index>```
4. It should look something like this:
``` MN1 127.0.0.2:11800 XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX X```
...
3. Enter ```armageddon-cli masternode status```
